...
{
    [CustomEditor(typeof(ImageModifier), true)]
    [CanEditMultipleObjects]
    public class ModifierImageEditor : ImageEditor
    {
        ...
        protected override void OnEnable()
        {
            ...
            attrList = ModifierTool.GetAttributeList();
            Debug.Log("Att Count " + attrList.Count);
            m_borderWidth = serializedObject.FindProperty("borderWidth");
            m_falloffDist = serializedObject.FindProperty("falloffDistance");

            if ((target as ImageModifier).GetComponent<BaseModifier>() != null)
            {
                selectedId = attrList.IndexOf(((ModifierID[])(target as ImageModifier).GetComponent<BaseModifier>().GetType().GetCustomAttributes(typeof(ModifierID), false))[0]);
            }
        ...
        public void UpdateProceduralImage()
        {
            if (target != null)
            {
                (target as ImageModifier).Update();
            }
            else
            {
                EditorApplication.update -= UpdateProceduralImage;
        ...
        protected void ModifierGUI()
        {
            ...
            if (targets.Length > 1)
            {
                Type t = (targets[0] as ImageModifier).GetComponent<BaseModifier>().GetType();
                foreach (var item in targets)
                {
                    if ((item as ImageModifier).GetComponent<BaseModifier>().GetType() != t)
                    {
            ...
            if (!hasMultipleValues)
            {
                ...
                if (selectedId != index)
                {
                    ...
                    foreach (var item in targets)
                    {
                        (item as ImageModifier).ModifierType = ModifierTool.GetTypeWithId(attrList[selectedId].Name);

                        MoveComponentBehind((item as ImageModifier), (item as ImageModifier).GetComponent<BaseModifier>());
                    }
        ...
        {
            ImageModifier image = target as ImageModifier;
